...
var COMMANDS = {
		...
		nicks.forEach(function (nick) {
			if (nick==SYSTEMNAME) {
				...
				if (myNick!=SYSTEMNAME) {
					pushMessage({ nick: SYSTEMSTRING, text: SYSTEMNAME + " ONLINE", admin: true})
				} else {
					pushMessage({ nick: "SYSTEM", text: "ONLINE", admin: true})
				}
			}
			if (nick.slice(0,4)!="FILT"&&nick!=SYSTEMNAME) {
				userAdd(nick);
				newNicks.push(nick)
			}
		});
	...
	onlineAdd: function (args) {
		...
		if ($('#joined-left').checked) {
			pushMessage({ nick: SYSTEMSTRING, text: nick + " joined", admin: true });
		}
	},
	...
	onlineRemove: function (args) {
		...
		if ($('#joined-left').checked) {
			pushMessage({ nick: SYSTEMSTRING, text: nick + " left", admin: true });
		}
	},
...
function pushMessage(args) {
	...
	if (verifyNickname(myNick) && args.nick == myNick && args.nick != SYSTEMNAME) {
		messageEl.classList.add('me');
	} else if (args.nick == '!') {
		messageEl.classList.add('warn');
	} else if (args.nick == SYSTEMNAME) {
		messageEl.classList.add('info');
	} else if (args.admin) {
